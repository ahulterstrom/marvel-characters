...
export async function getCharacterData(i) {
    ...
    try{
    const fullurl = "http://gateway.marvel.com/v1/public/characters?limit=10&offset=" + i + "&ts=" + ts + "&apikey=" + publicKey + "&hash=" + hash;
    console.log(fullurl);
...
export async function getCharacterPageData(name) {
    ...
    const fullurl = "http://gateway.marvel.com/v1/public/characters?name=" + searchName + "&ts=" + ts + "&apikey=" + publicKey + "&hash=" + hash;
    console.log(fullurl);
    try{
    const res = await fetch(fullurl, {cache: "force-cache"})
    var resJSON = await res.json();
