...
const publicKey = process.env.MARVEL_PUBLIC_KEY_ENV_LOCAL_VARIABLE;
...
export async function getCharacterData(i) {
  ...
  try {
    const fullurl =
      'http://gateway.marvel.com/v1/public/characters?limit=10&offset=' +
      i +
      '&ts=' +
      ts +
      '&apikey=' +
      ...
      '&hash=' +
      hash;
    // const fullurl = `http://offset=${i}&ts=${ts}`
    console.log(fullurl);
  ...
  return get(resJSON, 'data.results', []);
}

export async function getCharacterPageData(name) {
  const ts = Date.now();
  const hash = md5(ts + privateKey + publicKey).toString();
  let searchName = name.slice();
  ...
  try {
    ...
    pageData = await get(resJSON, 'data.results', []);
  } catch (error) {
    console.log(error);
  }
